...
import { ArrowRight, ArrowLeft, Plus, Trash2, HelpCircle, Target } from 'lucide-react';
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <span className="text-[12px] text-foreground-tertiary">{design.researchQuestions.length} question{design.researchQuestions.length !== 1 ? 's' : ''}</span>
          </div>
        </CardHeader>
            ...
            <div className="space-y-2">
              {design.researchQuestions.map((rq, i) => (
                <div key={i} className="flex items-start gap-2 p-3 rounded-md bg-blue-50/50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/30 group">
                  <span className="text-[11px] font-semibold text-blue-500 mt-0.5 shrink-0">RQ{i + 1}</span>
                  <p className="text-[13px] flex-1 leading-snug">{rq}</p>
                  <button
                    onClick={() => removeRQ(i)}
                    className="opacity-0 group-hover:opacity-100 text-foreground-tertiary hover:text-destructive transition-all"
                  >
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex gap-2">
            <Input
              ...
              placeholder={objectiveType === 'general'
          ...
          <p className="text-[11px] text-foreground-tertiary">
            Tip: Start with 1 general objective, then 3-5 specific objectives. Each should be SMART (Specific, Measurable, Achievable, Relevant, Time-bound).
          </p>
        </CardContent>
      </Card>
