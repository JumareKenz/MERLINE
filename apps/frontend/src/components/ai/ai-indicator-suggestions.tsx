...
interface AiIndicatorSuggestionsProps {
  onAdd: (suggestion: string) => void;
  context?: Record<string, unknown>;
  className?: string;
}

export function AiIndicatorSuggestions({ onAdd, context, className }: AiIndicatorSuggestionsProps) {
  const [text, setText] = useState('');
  const [showResults, setShowResults] = useState(false);
  const assistMutation = useAiAssist('suggestIndicators');
      ...
      {showResults && (
        <div className="rounded-lg border p-3">
          {assistMutation.isPending ? (
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold uppercase text-muted-foreground">Suggested Indicators</p>
